...
const createUser = async function (req, res) {
  try {
    ...
    if (isValidRequestBody(other))
      return res
        .status(400)
        .send({ status: false, message: "Any extra field is not allowed " });
    ...
    if (!isValid(fname)) {
      res
        .status(400)
        .send({ status: false, Message: "Please provide user's first name" });
      return;
    }
    if (!isValid(lname)) {
      res
        .status(400)
        .send({ status: false, Message: "Please provide user's last name" });
      return;
    }
    if (!isValid(email)) {
      res
    ...
    if (!isvalidaddress(address)) {
      res
        ...
        .send({
          status: false,
          Message:
            "Please provide address, it should contain shipping and billing address",
        });
      return;
    }
    if (address) {
  ...
  try {
    ...
    if (requestBody.email && requestBody.password) {
      ...
      if (!userEmail) {
        return res
          .status(400)
          .send({ status: false, msg: "Invalid user email" });
      }
      ...
      let generatedToken = jwt.sign(payload, "Group68", { expiresIn: "60m" });
      ...
      res
        .status(400)
        .send({ status: false, msg: "must contain email and password" });
    }
  } catch (error) {
...
const updateuser = async (req, res) => {
  ...
  if (address || address == "") {
    if (!isvalidaddress(address)) {
      return res
        .status(400)
        .send({ status: false, Message: "Not valid address" });
    }
